/**
 * Server-side access policy for ProofMark ShowAd. Direct port of the Next SDK
 * access-policy module, decoupled from `next/server` types so it can run inside
 * the Remix app's app-proxy routes.
 *
 * Pipeline: verified-crawler -> CIDR allowlist -> beforeProtect -> continue.
 */
...
export type AccessPolicyDecision =
  | AccessPolicyAction
  | { action: AccessPolicyAction; reason?: string; redirectUrl?: string };
...
export interface AccessPolicyOptions<RequestLike = unknown> {
  ...
  beforeProtect?: (
    ctx: AccessPolicyContext<RequestLike>
...
export async function evaluateAccessPolicy<R extends RequestLike>(
  request: R,
  options: AccessPolicyOptions<R> = {}
): Promise<Exclude<AccessPolicyDecision, AccessPolicyAction>> {
  const pathname = request.nextUrl?.pathname || tryUrlPathname(request.url) || '/';
  const ctx: AccessPolicyContext<R> = {
    request,
    pathname,
  ...
  if (crawler.verified) return { action: 'allow', reason: `crawler:${crawler.family}` };

  if (ctx.clientIp && isIpInCidrs(ctx.clientIp, options.allowCidrs || [])) {
    return { action: 'allow', reason: 'cidr_allowlist' };
  }

  if (options.beforeProtect) return normalizeDecision(await options.beforeProtect(ctx));
...
export async function verifyCrawlerRequest(
  ...
  if (!policy?.enabled) return { verified: false, reason: 'disabled' };

  const family = detectCrawlerFamily(input.userAgent || '', policy.families);
  if (!family) return { verified: false, reason: 'no_family_match' };

  const ip = (input.ip || '').trim();
  if (!ip) return { verified: false, family, reason: 'missing_ip' };
...
function detectCrawlerFamily(
  ...
  return (
    allowedFamilies.find((family) =>
      crawlerUserAgents[family]?.some((pattern) => pattern.test(userAgent))
    ) || null
  );
}

function normalizeDecision(decision: AccessPolicyDecision): Exclude<AccessPolicyDecision, AccessPolicyAction> {
  return typeof decision === 'string' ? { action: decision } : decision;
}
...
function tryUrlPathname(url?: string): string | null {
  if (!url) return null;
  try {
...
function ipMatchesCidr(ip: string, cidr: string): boolean {
  ...
  if (!ipParsed || !rangeParsed || ipParsed.bits !== rangeParsed.bits) return false;

  const prefix = bits === undefined ? ipParsed.bits : Number(bits);
  if (!Number.isInteger(prefix) || prefix < 0 || prefix > ipParsed.bits) return false;
...
function parseIp(ip: string): { value: bigint; bits: 32 | 128 } | null {
  const v = ip.trim();
  if (v.includes(':')) return parseIpv6(v);
  return parseIpv4(v);
}
