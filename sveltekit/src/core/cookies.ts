...
import type { CookieSetOptions, SvelteKitCookiesLike } from '../types';
...
/**
 * Apply verification cookies to a SvelteKit-shaped cookies API.
 * Mirrors `buildVerificationSetCookieHeaders` so both code paths stay aligned.
 */
export function applyVerificationCookies(
  cookies: SvelteKitCookiesLike,
  input: VerificationCookieInput
): void {
  const names = getCookieNames(input.cookiePrefix);
  ...
  cookies.set(names.token, input.token, { ...base, httpOnly: true });
  cookies.set(names.verified, '1', base);
  cookies.set(names.creator, input.creatorHash, base);
  if (input.ticketId) cookies.set(names.ticket, input.ticketId, base);
  if (input.tokenExpiry !== null) {
    cookies.set(names.expires, String(input.tokenExpiry), base);
  }
}

export function applyClearCookies(
  cookies: SvelteKitCookiesLike,
  cookiePrefix?: string
): void {
  const names = getCookieNames(cookiePrefix);
  const opts: CookieSetOptions = { path: '/' };
  cookies.delete(names.token, opts);
  cookies.delete(names.verified, opts);
  cookies.delete(names.creator, opts);
  cookies.delete(names.ticket, opts);
  cookies.delete(names.expires, opts);
}

/**
 * Append a set of `Set-Cookie` header strings to a Response's headers.
 * Mutates the headers (which are mutable on a constructed Response).
 */
export function appendSetCookieHeaders(headers: Headers, setCookies: string[]): void {
