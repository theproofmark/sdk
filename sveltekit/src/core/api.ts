/**
 * Backend HTTP client (uses global fetch from Node 18+ / SvelteKit runtime).
 */
...
function debugLog(config: Pick<ShowAdConfig, 'debug'>, ...args: unknown[]): void {
  if (config.debug) {
    ...
    console.log('[ShowAd SvelteKit SDK]', ...args);
  }
}
...
/**
 * Claim a redirect ticket and exchange it for a JWT. Called when the visitor
 * is redirected back from the video ad with `?redirect_ticket=<id>`.
 */
